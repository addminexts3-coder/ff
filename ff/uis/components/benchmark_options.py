...
from ff.types import BenchmarkMode, BenchmarkResolution
...
def render() -> None:
	...
	BENCHMARK_MODE_DROPDOWN = gradio.Dropdown(
		...
		choices = ff.choices.benchmark_modes,
	...
	BENCHMARK_RESOLUTIONS_CHECKBOX_GROUP = gradio.CheckboxGroup(
		...
		choices = ff.choices.benchmark_resolutions,
	...
	BENCHMARK_CYCLE_COUNT_SLIDER = gradio.Slider(
		...
		step = calculate_int_step(ff.choices.benchmark_cycle_count_range),
		minimum = ff.choices.benchmark_cycle_count_range[0],
		maximum = ff.choices.benchmark_cycle_count_range[-1]
	)
