...
from ff.ffmpeg import get_available_encoder_set
from ff.filesystem import is_image, is_video
from ff.types import AudioEncoder, Fps, Scale, VideoEncoder, VideoPreset
from ff.uis.core import get_ui_components, register_ui_component
from ff.vision import detect_video_fps
...
def render() -> None:
	...
	OUTPUT_IMAGE_QUALITY_SLIDER = gradio.Slider(
		...
		step = calculate_int_step(ff.choices.output_image_quality_range),
		minimum = ff.choices.output_image_quality_range[0],
		maximum = ff.choices.output_image_quality_range[-1],
	...
	OUTPUT_IMAGE_SCALE_SLIDER = gradio.Slider(
		...
		step = calculate_float_step(ff.choices.output_image_scale_range),
		value = state_manager.get_item('output_image_scale'),
		minimum = ff.choices.output_image_scale_range[0],
		maximum = ff.choices.output_image_scale_range[-1],
		visible = is_image(state_manager.get_item('target_path'))
	)
	...
	OUTPUT_AUDIO_QUALITY_SLIDER = gradio.Slider(
		...
		step = calculate_int_step(ff.choices.output_audio_quality_range),
		minimum = ff.choices.output_audio_quality_range[0],
		maximum = ff.choices.output_audio_quality_range[-1],
		visible = is_video(state_manager.get_item('target_path'))
	)
	OUTPUT_AUDIO_VOLUME_SLIDER = gradio.Slider(
		...
		step = calculate_int_step(ff.choices.output_audio_volume_range),
		minimum = ff.choices.output_audio_volume_range[0],
		maximum = ff.choices.output_audio_volume_range[-1],
		visible = is_video(state_manager.get_item('target_path'))
	)
	...
	OUTPUT_VIDEO_PRESET_DROPDOWN = gradio.Dropdown(
		...
		choices = ff.choices.output_video_presets,
	...
	OUTPUT_VIDEO_QUALITY_SLIDER = gradio.Slider(
		...
		step = calculate_int_step(ff.choices.output_video_quality_range),
		minimum = ff.choices.output_video_quality_range[0],
		maximum = ff.choices.output_video_quality_range[-1],
		visible = is_video(state_manager.get_item('target_path'))
	)
	OUTPUT_VIDEO_SCALE_SLIDER = gradio.Slider(
		label = translator.get('uis.output_video_scale_slider'),
		step = calculate_float_step(ff.choices.output_video_scale_range),
		value = state_manager.get_item('output_video_scale'),
		minimum = ff.choices.output_video_scale_range[0],
		maximum = ff.choices.output_video_scale_range[-1],
		visible = is_video(state_manager.get_item('target_path'))
	)
