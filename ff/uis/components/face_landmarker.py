...
import ff.choices
from ff import face_landmarker, state_manager, translator
from ff.common_helper import calculate_float_step
from ff.types import FaceLandmarkerModel, Score
...
def render() -> None:
	...
	FACE_LANDMARKER_MODEL_DROPDOWN = gradio.Dropdown(
		...
		choices = ff.choices.face_landmarker_models,
	...
	FACE_LANDMARKER_SCORE_SLIDER = gradio.Slider(
		...
		step = calculate_float_step(ff.choices.face_landmarker_score_range),
		minimum = ff.choices.face_landmarker_score_range[0],
		maximum = ff.choices.face_landmarker_score_range[-1]
	)
