...
import ff.choices
from ff import content_analyser, face_classifier, face_detector, face_landmarker, face_masker, face_recognizer, state_manager, translator, voice_extractor
from ff.filesystem import get_file_name, resolve_file_paths
from ff.processors.core import get_processors_modules
from ff.types import DownloadProvider
...
def render() -> None:
	...
	DOWNLOAD_PROVIDERS_CHECKBOX_GROUP = gradio.CheckboxGroup(
		...
		choices = ff.choices.download_providers,
...
def update_download_providers(download_providers : List[DownloadProvider]) -> gradio.CheckboxGroup:
	...
	available_processors = [ get_file_name(file_path) for file_path in resolve_file_paths('ff/processors/modules') ]
	...
	download_providers = download_providers or ff.choices.download_providers
